...
inlineVectorizedStringIndexOf(TR::Node* node, TR::CodeGenerator* cg, bool isUTF16)
   ...
   if (isUTF16 && cg->comp()->target().is64Bit())
      {
      generateShiftThenKeepSelected64Bit(node, cg, s1LenReg, s1LenReg, 31, 62, 1);
      generateShiftThenKeepSelected64Bit(node, cg, s2LenReg, s2LenReg, 31, 62, 1);
      generateShiftThenKeepSelected64Bit(node, cg, fromIndexReg, fromIndexReg, 31, 62, 1);
      }
   else
      {
